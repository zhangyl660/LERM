...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        if config["method"]=="BNM":
            ...
            # transfer_loss = entropy(softmax_tgt, reduction='mean')
            ...
            transfer_loss = criterion((H.T / a).T, center_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', type=int, default=1024, help="batch size")
    ...
    config["out_file"] = open(osp.join(config["output_path"], "log_test.txt"), "w")
    if not osp.exists(config["output_path"]):
