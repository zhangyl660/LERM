...
def main(args: argparse.Namespace, eps=0.):
    ...
    if args.dataset_ratio == -1:
        ...
        train_source_dataset, train_target_dataset, val_dataset, test_dataset, num_classes, args.class_names = \
            utils.get_ratio_dataset(args.data, args.root, args.source,
                                    args.target, train_transform, val_transform, args.dataset_ratio)
    ...
    if args.phase == 'analysis':
        ...
        # plot t-SNE
        # tSNE_filename = osp.join(logger.visualize_directory, f'{"".join(args.source)}2{"".join(args.target)}TSNE.png')
    ...
    for epoch in range(args.epochs):
        ...
        # [acc, loss] = train(train_source_iter, train_target_iter, classifier, domain_adv, mcc_loss, optimizer,
        #                     ad_optimizer,
        ...
        if acc1 > best_acc1:
            shutil.copy(logger.get_checkpoint_path('latest'),
                        logger.get_checkpoint_path(f'best_with_eps{eps}'))
        best_acc1 = max(acc1, best_acc1)
    # all_accuracy = np.array(all_accuracy)
    # np.save(os.path.join(args.log, "acc_and_loss"), all_accuracy)
    print("best_acc1 = {:3.2f}".format(best_acc1))

    # evaluate on test set
    classifier.load_state_dict(torch.load(logger.get_checkpoint_path(f'best_with_eps{eps}')))
    acc1 = utils.validate(test_loader, classifier, args, device)
    os.rename(logger.get_checkpoint_path(f'best_with_eps{eps}'),
              logger.get_checkpoint_path(f'best_{"".join(args.source)}2{"".join(args.target)}_eps{eps}_acc={acc1}'))
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
    ...
    CCL_losses = AverageMeter('CCL Loss', ':3.2f')
    cls_accs = AverageMeter('Cls Acc', ':3.1f')
    domain_accs = AverageMeter('Domain Acc', ':3.1f')
    progress = ProgressMeter(
        args.iters_per_epoch,
        [batch_time, data_time, losses, trans_losses, CCL_losses, cls_accs, domain_accs],
    ...
    for i in range(args.iters_per_epoch):
        ...
        softmax_tgt = nn.Softmax(dim=1)(y_t)
        ...
        loss = cls_loss + transfer_loss * args.trade_off + method_loss * args.lambda_method
        ...
        CCL_losses.update(method_loss.item(), x_s.size(0))
        ...
        if i % args.print_freq == 0:
            progress.display(i)
    # return [cls_accs.avg.cpu().numpy(), losses.avg]


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='CDAN+MCC with SDAT + EntMin for Unsupervised Domain Adaptation')
    # dataset parameters
    ...
    parser.add_argument('--dataset-ratio', default=-1, type=float,
                        help='The ratio between dataset1 and dataset2, it will crop the last dataset in the given '
                             'datasets rate')
    ...
    parser.add_argument('--lambda_method', default=0.1, type=float,
                        help='the trade-off hyper-parameter for method loss')
    # training parameters
    ...
    parser.add_argument("--phase", type=str, default='train',
                        choices=['train', 'test', 'analysis', 'finetune', 'analysis_distance','re_label'],
    ...
    if args.eps >= 1:
        ...
        for i in range(len(epses)):
            print('eps as these epoch is', epses[i])
        ...
        print('eps as these epoch is', args.eps)
