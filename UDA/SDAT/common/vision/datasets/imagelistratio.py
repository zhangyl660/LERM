...
class ImageListRatio(datasets.VisionDataset):
    """A generic Dataset class for image classification with ratio, it satisfies the class distribution equilibrium
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, dataset_length,
    ...
    def parse_data_file(self, file_name: str) -> List[Tuple[str, int]]:
        """Parse file to data list with datasetlength

        Args:
            file_name (str): The path of data file
            return (list): List of (image path, class_index) tuples
        """
        each_class_len = int(self.dataset_length/len(self.classes))
        ...
        with open(file_name, "r") as f:
            ...
            for line in f.readlines():
                ...
                target = int(target)
                # 分配每个类的可能
                if class_count[target] < each_class_len-1:
